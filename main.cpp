...
int main()
{
  ...
  while (1)
  {
      ...
      if (!contours.empty())
      {
          ...
          //cv::drawContours(frame, hull, 0, cv::Scalar(0, 255, 0), 3);//This needs to commented
          if (hull[0].size() > 2)
          {
              ...
              for (size_t i = 0; i < validPoints.size(); i++)
              {
                  ...
                  cv::rectangle(frame, cv::Point(validPoints[i].x-18, validPoints[i].y-40),cv::Point(validPoints[i].x+18, validPoints[i].y+40),cv::Scalar(0,0,255),2 );
